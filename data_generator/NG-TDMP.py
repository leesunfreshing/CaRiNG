...
def stationary_local_uninvertable_z_tulda():
    ...
    lambda_ = 1
    path = os.path.join(root_dir, "seed%d" % seed)
    ...
    for i in range(length):
        if i < lags_indicator-lags:
            ...
            # import ipdb; ipdb.set_trace()
            x_t_tulda[:,-1] = x_t[:,-1]+lambda_ * indicator_value.squeeze() 
            #  add the indicator value to the last dimension of x_t, It is uninvertable
